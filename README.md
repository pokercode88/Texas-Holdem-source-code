# 德州扑克 (Texas Hold'em) 完整源码 | 德州撲克完整原始碼 | Complete Source Code， 德州扑克|  德州撲克|  扑克源码|  撲克源碼|  俱乐部 | 俱樂部 | 语音聊天|  語音聊天。

是和HHpoke德扑圈，wpk直接竞争的产品；德州撲克，德撲源碼，德州源碼，德州俱樂部源碼，德州遊戲源碼，德州錦標賽，撲克源碼

德州私人局代码，俱乐部源码，poker代码,约局源码，AOF，短牌，SNG，MTT，德州牛仔，奥马哈，大菠萝等玩法，TG：@alibabama401

## 📖 项目介绍 | 專案介紹

**简体中文：**
一套功能极其丰富的德州扑克游戏完整源码，包含 Unity 客户端 + C++ 服务端。
**支持8种玩法**：经典德州、短牌、奥马哈、大菠萝、德州牛仔、AOF、SNG、MTT。
含俱乐部/联盟系统、实时语音聊天等商业功能，产品精美，代码可靠。

**繁體中文：**
一套功能極其豐富的德州撲克遊戲完整原始碼，包含 Unity 客戶端 + C++ 伺服器端。
**支援8種玩法**：經典德州、短牌、奧馬哈、大菠蘿、德州牛仔、AOF、SNG、MTT。
含俱樂部/聯盟系統、即時語音聊天等商業功能，產品精美，程式碼可靠。

**English:**
A complete commercial-grade Texas Hold'em poker game source code with Unity client + C++ server.
**Supports 8 game modes**: Classic Texas, Short Deck, Omaha, Pineapple, Texas Cowboy, AOF, SNG, MTT.
Includes club/alliance system and real-time voice chat.
## ✨ 核心功能 | Core Features

| 模块 Module | 功能说明 Function |
|:---|:---|
| 🎰 **8种玩法** | 经典德州 / 短牌 / 奥马哈 / 大菠萝 / 德州牛仔 / AOF / SNG / MTT |
| 🎙️ **语音聊天** | 内置实时语音，朋友局体验更真实 / Built-in real-time voice chat |
| 🏛️ **俱乐部+联盟** | 创建俱乐部、多俱乐部联盟、排行榜 / Club & alliance system |
| 👥 **私人朋友局** | 密码房、好友邀请、观战模式 / Private rooms with password |
| 📊 **完整赛事系统** | SNG单桌淘汰赛 + MTT多桌锦标赛 |
| 🎨 **精美UI** | 产品精美，接近一线商业风格 / Commercial-grade UI design |

 ## ✨ 获取完整商业源码 | Contact
如需包含全部8种玩法、语音聊天、俱乐部联盟、完整代码及技术支持的商业版本，请联系：

Telegram	@alibabama401

Email	ttpoker733@gmail.com

## 🏗️ 技术架构 | Tech Stack

### 技术栈详情 | Technology Details

| 模块 Module | 技术 Technology |
|:---|:---|
| 客户端引擎 Client Engine | Unity 2019.4 / 2021.3 |
| 服务端语言 Server Language | C++17 |
| 网络框架 Network Framework | Tars + WebSocket |
| 数据库 Database | MySQL 8.0 + Redis 7.0 |
| 语音聊天 Voice Chat  |
| 部署方式 Deployment | Docker / 裸金属服务器 |


## 📸 界面截图 | Screenshots

![3联盟-加入主页](https://github.com/user-attachments/assets/205c0b50-5600-4ab4-a954-e361311a4b9a)
![3联盟-充值联盟币](https://github.com/user-attachments/assets/e0fc4fe0-61ca-480a-a099-dde20cf38f5a)
![03赛事4](https://github.com/user-attachments/assets/a755bbb1-70c0-4f3c-9c69-b89377dbbb59)

![02创建1](https://github.com/user-attachments/assets/acc0fa3a-d864-41a3-9d91-a669cf5de648)
![1管理俱乐部币](https://github.com/user-attachments/assets/722fa342-865b-410b-801b-e5cd11488d12)
![1房间列表](https://github.com/user-attachments/assets/4ac89b79-94de-4903-8a5a-ed080d5ec24d)
![01](https://github.com/user-attachments/assets/d1a5299a-ac9a-47e6-9206-d0ebae3a1dec)
![05牌桌-查看头像1](https://github.com/user-attachments/assets/349d9333-fe5c-46ca-913d-39bc35bc95e0)
![4房间2-9人桌](https://github.com/user-attachments/assets/0d709180-2259-45b4-9f99-17f0dcb41e3a)
![4房间2-6人桌](https://github.com/user-attachments/assets/1549d61a-2dc4-4059-ad86-a68464e78d88)


## 🚀 快速体验 | Quick Start

### 方式一：直接安装 APK（推荐）

1. 请点击下载：https://github.com/pokercode88/Texas-Holdem-source-code/releases/download/1.0%E7%89%88%E6%9C%AC/jinbei.client_4_0304.apk
2. 如需最新版本可以联系：Telegram	@alibabama401
3. 安装到 Android 手机
4. 登录体验账号：`test` / `123456`
5. 创建房间，开始游戏

### ❓ 常见问题 | FAQ
Q1: 这套源码包含哪些玩法？
A: 包含经典德州、短牌、奥马哈、大菠萝、德州牛仔、AOF、SNG、MTT，共8种玩法。

Q2: 代码质量如何？
A: 已稳定运营多年，代码结构清晰，注释完整。

Q3: 可以二次开发吗？
A: 可以。源码无加密，支持功能扩展和UI修改。

Q4: 支持多少人在线？
A: 单服支撑2000+并发，支持横向扩展。

Q5: 有后台管理吗？
A: 有。包含玩家管理、活动配置、公告推送、机器人控制等。

<div align="center">
如果这个项目对你有帮助，欢迎点个 Star 支持一下！
⭐️ Star ⭐️
